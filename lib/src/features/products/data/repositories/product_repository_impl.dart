...
class ProductRepositoryImpl implements ProductRepository {
  ProductRepositoryImpl({
    ...
    ProductLocalDataSource? localDataSource,
           ...
           localDataSource ??
           ProductLocalDataSourceImpl(database: database ?? AppDatabase.instance);
  ...
  @override
  Future<List<Product>> searchProducts(String query) async => (await _localDataSource
  ...
  @override
  Future<List<PriceHistoryEntry>> getProductPriceHistory(String productId) async =>
      (await _localDataSource.getProductPriceHistory(productId))
          .map((m) => m.toEntity())
          .toList();
